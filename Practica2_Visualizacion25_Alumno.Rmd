...
grafico1<-ggplot(data = ds)+
  geom_point(mapping = aes(x=MinTemp,y=MaxTemp))
grafico1

grafico2<-ggplot(data = ds)+
  geom_point(mapping = aes(x=MinTemp,y=MaxTemp,colour=RainTomorrow))
#Si quiero cambiar los colores manualmente sería grafico2 + scale_color_manual(values=c("blue","green"))
grafico2

grafico3<-ggplot(data = ds)+
  geom_point(mapping = aes(x=MinTemp,y=MaxTemp,colour=RainTomorrow))+
  ggtitle("Relación entre temperaturas máxima y mínima y probabilidad de lluvia")
...
grafico1.2<-ggplot(data = ds)+
  geom_point(mapping = aes(x=Temp9am,y=Pressure9am))
grafico1.2

grafico2.2<-ggplot(data = ds)+
  geom_point(mapping = aes(x=Temp9am,y=Pressure9am,shape=RainToday))
grafico2.2

grafico3.2<-ggplot(data = ds)+
  geom_point(mapping = aes(x=Temp9am,y=Pressure9am,colour=RainToday))
grafico3.2

#Visualmente es mejor representar las diferencias de RainToday con colores. Del otro modo no se aprecian tantas diferencias.

grafico4.2<-grafico3.2+labs(title ="Probabilidad lluvia según condiciones metereológicas",x="Temperatura 9AM °C",y="Presión atmosférica 9AM(mb)")
...
str(iris.tidy)
iris.grafico1<-ggplot(data=iris.tidy)+
  geom_point(mapping = aes(x=Species,y=Value))
iris.grafico1

iris.grafico2<-ggplot(data=iris.tidy)+
  geom_point(mapping = aes(x=Species,y=Value,colour=Part))
iris.grafico2

iris.grafico3<-ggplot(data=iris.tidy,aes(x=Species,y=Value,colour=Part))+
                        geom_point()+facet_grid(Measure~.)
iris.grafico3

iris.grafico4<-ggplot(data=iris.tidy,aes(x=Species,y=Value,colour=Part))+
                        geom_jitter()+facet_grid(Measure~.)
iris.grafico4

#Los datos del gráfico ya no se "sobreponen" uno sobre otro
...
grafico4.1<-ggplot(data=DatosEjer4)+
  geom_point(mapping = aes(x=Fecha,y=Ventas,colour = Fabricante))
grafico4.1

grafico4.2<-ggplot(data=DatosEjer4,aes(x=Fecha,y=Ventas,colour=Fabricante))+
  geom_point()+
  geom_line()
grafico4.2

grafico4.3<-ggplot(data=DatosEjer4,aes(x=Fecha,y=Ventas,colour=Fabricante,group=Fabricante))+
  geom_point()+
  geom_line(aes(linetype=Fabricante))
grafico4.3

grafico4.4<-grafico4.3+labs(title ="Ventas mensuales de diferentes compañías tecnológicas")
grafico4.4
...
grafico5.1<-ggplot(data=Experimento,aes(x=Dosis,y=Altura,shape=Complemento,group = Complemento))+
  geom_line(aes(linetype = Complemento))+
  geom_point()
grafico5.1

grafico5.2<-ggplot(data=Experimento,aes(x=Dosis,y=Altura,shape=Complemento,group = Complemento))+
  geom_line(aes(linetype = Complemento))+
  geom_point()+
  geom_errorbar(aes(ymin=Altura-Error_Altura,ymax=Altura+Error_Altura), width=.1, position=position_dodge(0.05))
grafico5.2

grafico5.3<-ggplot(data=Experimento,aes(x=Dosis,y=Altura,color=Complemento,group = Complemento))+
  geom_line(aes(linetype = Complemento))+
...
grafico6.1<-ggplot(data=ice.river.df)+
  geom_line(mapping = aes(x=time,y=flow.vat),colour="red")+
  geom_line(mapping = aes(x=time,y=flow.jok),colour="blue")
grafico6.1

grafico6.2<-grafico6.1+labs(title = "Caudales de los rios islandeses Vatnsdalsa(azul) y Jokulsa Eystri(rojo)",x="Tiempo (años)",y="Caudal")
grafico6.2
...
grafico7.1<-ggplot(data=ds,mapping=aes(x=Year,y=MinTemp))+
  geom_boxplot()
grafico7.1

grafico7.2<-ggplot(data=ds,mapping=aes(x=Year,y=MaxTemp,fill=Year))+
  geom_violin()
grafico7.2<-grafico7.2+labs(title="Distribución de temperaturas máximas por año",x="Año",y="Temperatura (ºC)")
grafico7.2<-grafico7.2+theme(axis.text.x = element_text(angle = 45, hjust = 1))
grafico7.2
...
rm(list=ls())
library(readr)
dataCamp<-read.table("data/chol.txt",header = T,sep = " ")
